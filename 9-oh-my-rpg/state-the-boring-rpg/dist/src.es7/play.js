...
function receive_stat_increase(state, stat, amount = 1) {
    return Object.assign({}, state, { avatar: increase_stat(get_SEC(), state.avatar, stat, amount) });
}
...
function play_good(state, explicit_adventure_archetype_hid) {
    ...
    state = Object.assign({}, state, { prng: register_recently_used(state.prng, ADVENTURE_NON_REPETITION_ID, aa.hid, 7) });
