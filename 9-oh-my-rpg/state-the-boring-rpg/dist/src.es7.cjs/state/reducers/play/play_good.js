...
function play_good(state, explicit_adventure_archetype_hid) {
    ...
    if (!aa)
        throw new Error(`${consts_1.LIB}: play_good(): hinted adventure archetype "${explicit_adventure_archetype_hid}" could not be found!`);
    ...
    state = Object.assign({}, play_adventure_1.play_adventure(state, aa), { good_click_count: state.good_click_count + 1 });
    return state;
}
