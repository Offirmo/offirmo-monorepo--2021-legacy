...
import { InventorySlot, ItemQuality, create_item_base, } from '@oh-my-rpg/definitions';
import { SCHEMA_VERSION } from './consts';
import { create, add_item, remove_item_from_unslotted, equip_item, get_equiped_item_count, get_unequiped_item_count, get_item_count, get_item_in_slot, iterables_unslotted, } from '.';
describe('📦 📦 📦  Inventory state - reducer', function () {
    const DUMMY_ITEM = create_item_base(InventorySlot.weapon);
    ...
    describe('📥 item addition', function () {
        ...
        it('should succeed slot when some items where recently removed', function () {
            ...
            const item2 = create_item_base(InventorySlot.weapon);
            let state = create();
    ...
    describe('📤 item removal', function () {
        ...
        it('should work in nominal case', function () {
            const item1 = create_item_base(InventorySlot.armor);
            const item2 = create_item_base(InventorySlot.weapon);
            let state = create();
            state = add_item(state, item1);
            state = add_item(state, item2);
    ...
    describe('⬆ item equipping', function () {
        ...
        it('should work on simple non-empty state and correctly swap if the slot was occupied', function () {
            let state = create();
            const item1 = create_item_base(InventorySlot.weapon, ItemQuality.uncommon);
            state = add_item(state, item1);
            ...
            expect(state.unslotted).to.have.lengthOf(0);
            const item2 = create_item_base(InventorySlot.weapon, ItemQuality.rare);
            state = add_item(state, item2);
    ...
    describe('misc items iteration', function () {
        it('should yield all unequiped slots', () => {
            const item1 = create_item_base(InventorySlot.armor);
            const item2 = create_item_base(InventorySlot.armor);
            const item3 = create_item_base(InventorySlot.weapon);
            let state = create();
            state = add_item(state, item1);
            state = add_item(state, item2);
