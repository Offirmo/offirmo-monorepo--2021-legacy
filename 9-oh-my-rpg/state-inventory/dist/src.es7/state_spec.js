...
import { create, add_item, remove_item, equip_item, unequip_item, get_equiped_item_count, get_unequiped_item_count, get_item_count, get_item_at_coordinates, get_item_in_slot, iterables_unslotted, } from '.';
describe('📦 📦 📦  Inventory state - reducer', function () {
    const EXPECTED_UNSLOTTED_INVENTORY_LENGTH = 20;
    const DUMMY_ITEM = Object.assign({}, create_element_base(ElementType.item), { slot: InventorySlot.none, quality: ItemQuality.common });
    const DUMMY_EQUIPABLE_ITEM = Object.assign({}, create_element_base(ElementType.item), { slot: InventorySlot.weapon, quality: ItemQuality.common });
    ...
    describe('📥 item addition', function () {
        ...
        it('should find a free slot when some items where recently removed', function () {
            ...
            const item2 = Object.assign({}, create_element_base(ElementType.item), { slot: InventorySlot.none, quality: ItemQuality.common });
            let state = create();
    ...
    describe('⬆ item equipping', function () {
        ...
        it('should work on simple non-empty state and correctly swap if the slot was occupied', function () {
            ...
            state = add_item(state, item1);
            state = equip_item(state, 0);
            const item2 = Object.assign({}, create_element_base(ElementType.item), { slot: InventorySlot.weapon, quality: ItemQuality.rare });
            state = add_item(state, item2);
            state = equip_item(state, 0);
    ...
    describe('misc items iteration', function () {
        it('should yield all unequiped slots', () => {
            const item1 = Object.assign({}, create_element_base(ElementType.item), { slot: InventorySlot.armor, quality: ItemQuality.common });
            const item2 = Object.assign({}, create_element_base(ElementType.item), { slot: InventorySlot.weapon, quality: ItemQuality.common });
            const item3 = Object.assign({}, create_element_base(ElementType.item), { slot: InventorySlot.none, quality: ItemQuality.common });
            let state = create();
            state = add_item(state, item1);
            state = add_item(state, item2);
