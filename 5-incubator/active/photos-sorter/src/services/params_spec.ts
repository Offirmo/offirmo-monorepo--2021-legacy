	...
	describe('get_default_timezone()', function() {
		...
		it('should work - real case', () => {
			test_params.default_timezones = [
				...
				//
				{
					date_utc_ms: Number(new Date(1826, 0)),
			...
			const default_tz_2001 = get_default_timezone(new Date(2001, 0), test_params)
			expect(default_tz_2001, '2001').to.equal('Europe/Paris')

			const default_tz_2009_08_09 = get_default_timezone(new Date(2009, 7, 9), test_params)
			expect(default_tz_2009_08_09).to.equal('Europe/Paris')
			const default_tz_2009_08_10 = get_default_timezone(new Date(2009, 7, 10), test_params)
			expect(default_tz_2009_08_10).to.equal('Asia/Bangkok')

			const default_tz_2010_07_08 = get_default_timezone(new Date(2010, 6, 8), test_params)
			expect(default_tz_2010_07_08).to.equal('Europe/Paris')

			const default_tz_2018 = get_default_timezone(new Date(2018, 0), test_params)
