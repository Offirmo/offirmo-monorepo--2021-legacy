...
function persist_pending_actions(SEC: SoftExecutionContext, storage: TbrpgStorage, pending_actions: Action[]): void {
...
function reset_pending_actions(SEC: SoftExecutionContext, storage: TbrpgStorage): void {
	return persist_pending_actions(SEC, storage, [])
}
...
function create(
	...
	let indirect_store = {
		set(new_state: Readonly<State>): void {
			reset_pending_actions(SEC, storage)
